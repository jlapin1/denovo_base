...
class D3PM:
    ...
    def model_predict(self, x_0, t, cond):
        ...
        predicted_x0_logits = self.x0_model(x_0, t, **cond)['out']
    ...
    def p_sample(self, x, t, model_kwargs, noise):

        predicted_x0_logits = self.model_predict(x, t, model_kwargs)
        ...
        return sample, predicted_x0_logits
    ...
    def _sample(self, x, model_kwargs={}, save_x=False, save_p=False, progress=False, **kwargs):
        ...
        if self.x0_model.self_condition:
            model_kwargs['self_conditions'] = torch.zeros(*x.shape, self.num_classes).type(torch.float32).to(x.device)
