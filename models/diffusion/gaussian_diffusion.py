...
class GaussianDiffusion:
    ...
    def _loss_history_update(self, ts, losses, loss_masks, training_step): #v5
        ...
        losses_gather_buffer = [th.zeros_like(losses)]
        #dist.all_gather(losses_gather_buffer, losses.detach())
        for loss in losses_gather_buffer: loss += losses.detach()
        ...
        loss_masks_gather_buffer = [th.zeros_like(loss_masks)]
        #dist.all_gather(loss_masks_gather_buffer, loss_masks.detach())
        for mask in loss_masks_gather_buffer: mask += loss_masks.detach()
        ...
        ts_gather_buffer = [th.zeros_like(ts)]
        #dist.all_gather(ts_gather_buffer, ts.detach())
        for gather in ts_gather_buffer: gather += ts.detach()
    ...
    def training_losses(self, model, training_step, ts, model_kwargs=None, noise=None):
        """
        ...
        input_ids = model_kwargs.pop("decoder_input_ids").to(ts.device)
        if 'loss_mask' in model_kwargs:
            loss_mask = model_kwargs.pop('loss_mask').to(ts.device)
        else:
            loss_mask = None
        x_start_mean = model.model.get_embed(input_ids)
        ...
        x_t = self.q_sample(x_start, ts, noise=noise)  # reparametrization trick.
